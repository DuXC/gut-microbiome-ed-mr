...
figure_library <- "/opt/homebrew/lib/R/4.6/site-library"
available_libraries <- c(local_library, figure_library)
available_libraries <- available_libraries[dir.exists(available_libraries)]
.libPaths(c(available_libraries, .libPaths()))
...
derived_v032 <- file.path(project_root, "05_results", "v0_3_2_derived_20260723")
...
hunt_unique_summary <- fread(file.path(
  derived_v032, "hunt_same_snp_unique_snp_summary.csv"
))
sensitivity_locus_audit <- fread(file.path(
  derived_v032, "ed_2025_sensitivity_trait_locus_audit.csv"
))
sensitivity_locus_summary <- fread(file.path(
  derived_v032, "ed_2025_sensitivity_locus_summary.csv"
))

# Figure 1 ---------------------------------------------------------------------
figure1_counts <- data.table(
  direction = c(
    rep("Forward", 6), rep("Cross-cohort same-SNP evaluation", 5),
    rep("Reverse", 5), rep("Alternative ED outcome sensitivity", 4)
  ),
  item = c(
    "Swedish traits", "Eligible forward traits", "Estimable forward traits",
    "Forward nominal", "Forward FDR", "FinnGen cases and controls",
    "Exact-label traits", "Same-SNP coverage", "Trait-row concordant",
    "Unique-SNP concordant", "Focal GWS HUNT instruments",
    ...
    "EUR high-LD sensitivity loci"
  ),
  value = c(
    1572, 230, 218, 7, 0, 2886 + 215272,
    97, 97, 76, 69, 0, 479, 24, 1572, 77, 0, 8, 0, 8, 1
  ),
  evidence_level = c(
    rep("primary discovery", 6), rep("independent exposure cohort only", 5),
    rep("primary reverse screen", 5),
    "known FinnGen overlap", "no FinnGen overlap; ancestry-transfer limited",
    "known FinnGen overlap", "derived EUR LD compression"
  )
)
...
draw_figure_1 <- function() {
  ...
  draw_box(0.14, 0.84, 0.22, 0.13, "Swedish shotgun GWAS",
           "1 572 microbial traits\nN = 16 017; European")
  draw_box(0.39, 0.84, 0.18, 0.13, "Primary instruments",
           "P < 5×10⁻⁸; F > 10\n230 eligible traits")
  draw_box(0.62, 0.84, 0.19, 0.13, "FinnGen R12 ED",
           "2 886 cases\n215 272 controls")
  draw_box(0.86, 0.84, 0.19, 0.13, "Forward result",
           "218 estimable\n7 nominal; 0 FDR")
  draw_arrow(0.25, 0.84, 0.30, 0.84)
  draw_arrow(0.48, 0.84, 0.525, 0.84)
  draw_arrow(0.715, 0.84, 0.765, 0.84)

  draw_box(0.16, 0.63, 0.27, 0.13, "Exact-label HUNT coverage",
           "97 rows; 90 unique lead rsIDs", "white")
  draw_box(0.50, 0.63, 0.30, 0.13,
           "Cross-cohort same-SNP evaluation",
           "76/97 rows; 69/90 unique rsIDs\ndirection concordant; scales differ", "white")
  draw_box(0.84, 0.63, 0.27, 0.13, "Exploratory HUNT-selected MR",
           "P<1×10⁻⁵ (23/20 SNPs)\n0 focal GWS HUNT instruments",
           "white")
  draw_arrow(0.14, 0.775, 0.16, 0.70, lty = 2L)
  draw_arrow(0.295, 0.63, 0.35, 0.63, lty = 2L)
  grid::grid.lines(
    x = grid::unit(c(0.16, 0.16, 0.84, 0.84), "npc"),
    y = grid::unit(c(0.565, 0.54, 0.54, 0.565), "npc"),
    arrow = grid::arrow(type = "closed", length = grid::unit(2.3, "mm")),
    gp = grid::gpar(col = palette[["neutral_dark"]], lwd = 1.2, lty = 2)
  )

  draw_box(0.50, 0.42, 0.74, 0.16, "2025 ED outcome sensitivities",
           paste0(
             "European: 8 FDR-significant trait rows | African ancestry: 0 | Cross-ancestry: 8\n",
             "separate 230-trait BH families; non-estimable rows retained at P=1\n",
             "same 8 EUR/cross rows; 3 lead SNPs in 1 high-LD chr2q21 locus\n",
             "EUR/cross include FinnGen; AFR does not, but has ancestry-transfer limitations"
           ), "white")
  grid::grid.lines(
    x = grid::unit(c(0.48, 0.685, 0.685), "npc"),
    y = grid::unit(c(0.775, 0.745, 0.505), "npc"),
    arrow = grid::arrow(type = "closed", length = grid::unit(2.3, "mm")),
    gp = grid::gpar(col = palette[["neutral_dark"]], lwd = 1.2, lty = 3)
  )

  grid::grid.lines(
    x = grid::unit(c(0.03, 0.97), "npc"),
    y = grid::unit(c(0.315, 0.315), "npc"),
    gp = grid::gpar(col = palette[["neutral_mid"]], lwd = 1.0)
  )
  grid::grid.text(
    "b", x = 0.018, y = 0.290, just = c("left", "top"),
    gp = grid::gpar(fontfamily = font_family, fontsize = 10, fontface = "bold")
  )
  grid::grid.text(
    "Reverse: erectile dysfunction to gut microbial traits",
    x = 0.055, y = 0.284, just = c("left", "top"),
    gp = grid::gpar(fontfamily = font_family, fontsize = 9.3, fontface = "bold")
  )
  draw_box(0.14, 0.18, 0.22, 0.11, "2025 European ED GWAS",
           "136 867 cases\n776 327 controls")
  draw_box(0.39, 0.18, 0.18, 0.11, "ED instruments",
           "479 GWS variants\n24 independent IVs")
  draw_box(0.62, 0.18, 0.19, 0.11, "Swedish outcomes",
           "1 572 microbial traits\n15–17 IVs per trait")
  draw_box(0.86, 0.18, 0.19, 0.11, "Reverse result",
           "1 572 estimable\n77 nominal; 0 FDR")
  draw_arrow(0.25, 0.18, 0.30, 0.18)
  draw_arrow(0.48, 0.18, 0.525, 0.18)
  draw_arrow(0.715, 0.18, 0.765, 0.18)
  grid::grid.text(
    "Eligibility and multiplicity rules were defined before association screening.",
    x = 0.50, y = 0.073,
  ...
  grid::grid.text("primary analysis", x = 0.12, y = 0.012, just = "left",
  ...
  grid::grid.text("HUNT exposure cohort", x = 0.37, y = 0.012,
                  just = "left",
                  gp = grid::gpar(fontfamily = font_family, fontsize = 5.9))
  grid::grid.lines(
  ...
  grid::grid.text("alternative ED outcome", x = 0.69, y = 0.012,
                  just = "left",
                  gp = grid::gpar(fontfamily = font_family, fontsize = 5.9))
}

save_grid(
  draw_figure_1, "IJIR_Figure_1_Study_Design_v0_3_5", 183, 160,
...
save_gg(
  figure2, "IJIR_Figure_2_Multiplicity_v0_3_5", 183, 88,
...
p3a_forest <- ggplot(nominal, aes(y = y)) +
  ...
  labs(
    ...
    subtitle = "All are single-SNP Wald ratios; none survived FDR; grey band = one nested taxonomic signal cluster",
...
p3c_power <- ggplot(power_plot, aes(alpha_display, mde_or_higher)) +
  ...
  labs(title = "Minimum detectable OR per\nstandardized trait unit", x = NULL,
       y = "80%-power MDE OR",
       subtitle = "Presence traits use an approximate\nliability-scale SD") +
  theme_ijir(6.4) +
  theme(plot.margin = margin(4, 2, 2, 2))
...
save_gg(
  figure3, "IJIR_Figure_3_Forward_Evidence_Profile_v0_3_5", 183, 156,
  source_files = c(
...
s1c <- ggplot(power_nominal, aes(phenotype_display, r2, fill = phenotype_display)) +
  ...
  labs(tag = "c", title = "Approximate exposure variance explained",
       ...
       subtitle = "Forward-trait scale; assumptions in Table S5") +
  theme_ijir() +
  theme(legend.position = "none", axis.text.x = element_text(angle = 20, hjust = 1))
supp_s1 <- s1a + s1b + s1c + plot_layout(widths = c(0.85, 0.95, 1.20))
save_gg(
  supp_s1, "IJIR_Supplementary_Figure_S1_Instrument_Architecture_v0_3_5",
...
save_gg(
  supp_s2, "IJIR_Supplementary_Figure_S2_Pvalue_Calibration_v0_3_5",
...
s3_summary_source <- write_source(
  hunt_unique_summary, "Supplementary_Figure_S3_concordance_source_data.csv"
)
s3a <- ggplot(hunt, aes(swedish_signed_z, hunt_signed_z_aligned,
  ...
  labs(tag = "a", title = "Exact-label same-SNP exposure associations",
       ...
       subtitle = "97/97 exact-label Swedish lead rsIDs found; no fuzzy matching") +
  theme_ijir() + theme(legend.position = "bottom")
concordance <- copy(hunt_unique_summary)
concordance[, analysis_label := factor(
  analysis,
  levels = rev(c("trait-level exact-label same-SNP evaluation", "unique-SNP sensitivity")),
  labels = rev(c("Trait rows", "Unique rsIDs"))
)]
concordance[, result_label := sprintf(
  "%d/%d = %.1f%%\n95%% CI %.1f%%–%.1f%%",
  direction_concordant, denominator, 100 * proportion,
  100 * exact_binomial_ci_lower, 100 * exact_binomial_ci_upper
)]
s3b <- ggplot(concordance, aes(proportion, analysis_label)) +
  geom_vline(xintercept = 0.5, linetype = "dashed", linewidth = 0.40,
             colour = palette[["neutral_mid"]]) +
  geom_segment(aes(x = exact_binomial_ci_lower,
                   xend = exact_binomial_ci_upper,
                   yend = analysis_label), linewidth = 0.55,
               colour = palette[["neutral_dark"]]) +
  geom_point(size = 3.0, shape = 21, fill = palette[["blue"]],
             colour = "black", stroke = 0.45) +
  geom_text(aes(label = result_label), nudge_y = 0.26,
            size = 2.2, family = font_family, fontface = "bold") +
  scale_x_continuous(limits = c(0.4, 1.02), labels = scales::label_percent(),
                     breaks = c(0.5, 0.6, 0.7, 0.8, 0.9, 1),
                     expand = expansion(mult = c(0.01, 0))) +
  labs(tag = "b", title = "Direction-concordance sensitivities",
       x = "Concordant proportion",
       y = NULL,
       subtitle = paste0(
         "Descriptive exact-binomial intervals;\n",
         "trait correlation not modelled"
       )) +
  theme_ijir() +
  theme(
    plot.subtitle = element_text(size = 5.9, lineheight = 1.08),
    plot.margin = margin(4, 9, 4, 3)
  )
supp_s3 <- s3a + s3b +
  plot_layout(widths = c(1.55, 1.10), guides = "collect") +
  plot_annotation(caption = paste0(
    "Signed Z supports a direction-only comparison. Swedish presence and HUNT normalized abundance scales preclude direct comparison of raw beta or MR OR magnitude."
             ...
             plot.caption = element_text(size = 5.8, hjust = 0))
save_gg(
  supp_s3, "IJIR_Supplementary_Figure_S3_HUNT_Same_SNP_v0_3_5",
  183, 108, supplement_dir, c(s3_source, s3_summary_source)
)
...
save_gg(
  supp_s4, "IJIR_Supplementary_Figure_S4_Source_Threshold_v0_3_5",
...
save_gg(
  supp_s5, "IJIR_Supplementary_Figure_S5_Mechanistic_Evidence_Map_v0_3_5",
  183, 106, supplement_dir, s5_source
)

# Supplementary Figure S6 ------------------------------------------------------
s6 <- copy(sensitivity_locus_audit)
s6[, short_trait := fcase(
  grepl("Bifidobacterium longum", trait), "B. longum abundance",
  grepl("pseudocatenulatum", trait), "B. pseudocatenulatum presence",
  grepl("Negativibacillus", trait), "Negativibacillus presence",
  grepl("Phocea massiliensis", trait), "P. massiliensis abundance",
  grepl("genus abundance \\(Phocea\\)", trait), "Phocea genus abundance",
  grepl("Xylose degradation", trait), "Xylose degradation function",
  grepl("Copromonas", trait), "Copromonas abundance",
  grepl("Lachnoclostridium", trait), "Lachnoclostridium presence",
  default = trait
)]
setorder(s6, lead_snp, short_trait)
s6[, y := .N:1L]
s6[, row_label := paste0(short_trait, "  [", lead_snp, "]")]
s6_long <- rbindlist(list(
  s6[, .(source_id, row_label, lead_snp, y, outcome = "FinnGen primary",
         beta = finngen_beta, se = finngen_se, p = finngen_p, q = finngen_q)],
  s6[, .(source_id, row_label, lead_snp, y, outcome = "2025 European",
         beta = eur_beta, se = eur_se, p = eur_p, q = eur_q)],
  s6[, .(source_id, row_label, lead_snp, y, outcome = "2025 African ancestry",
         beta = afr_beta, se = afr_se, p = afr_p, q = afr_q)],
  s6[, .(source_id, row_label, lead_snp, y, outcome = "2025 cross-ancestry",
         beta = cross_beta, se = cross_se, p = cross_p, q = cross_q)]
))
s6_long[, outcome := factor(
  outcome,
  levels = c("FinnGen primary", "2025 European", "2025 African ancestry",
             "2025 cross-ancestry")
)]
s6_long[, `:=`(
  signed_z = beta / se,
  estimability = fifelse(is.finite(beta) & is.finite(se), "Estimated", "Not estimable")
)]
s6_long[, plot_z := fifelse(estimability == "Estimated", signed_z, 0)]
s6_rect <- s6[, .(
  ymin = min(y) - 0.45, ymax = max(y) + 0.45,
  fill = fifelse(.GRP %% 2L == 1L, "#F3F3F3", "#FFFFFF")
), by = lead_snp]
s6_source1 <- write_source(
  s6_long, "Supplementary_Figure_S6a_ancestry_source_data.csv"
)
s6_source2 <- write_source(
  sensitivity_locus_summary,
  "Supplementary_Figure_S6b_locus_source_data.csv"
)
s6a <- ggplot(s6_long, aes(plot_z, y)) +
  geom_rect(
    data = s6_rect,
  ...
  scale_fill_identity() +
  geom_vline(xintercept = 0, linewidth = 0.40, colour = "black") +
  geom_vline(xintercept = c(-1.96, 1.96), linewidth = 0.35,
             linetype = "dotted", colour = palette[["neutral_mid"]]) +
  geom_point(aes(shape = estimability), size = 1.8, stroke = 0.45,
             colour = palette[["blue"]]) +
  geom_text(
    data = s6_long[estimability == "Not estimable"],
    aes(label = "not estimable"), nudge_x = 0.15, hjust = 0,
    size = 1.55, family = font_family, colour = palette[["neutral_mid"]]
  ) +
  facet_wrap(~outcome, nrow = 1) +
  scale_shape_manual(values = c("Estimated" = 16, "Not estimable" = 4)) +
  scale_x_continuous(limits = c(-4.6, 4.6), breaks = c(-4, -2, 0, 2, 4)) +
  scale_y_continuous(breaks = s6$y, labels = s6$row_label) +
  labs(
    tag = "a", title = "Trait-level direction and statistical strength across ED outcomes",
    subtitle = "Signed Z is shown for direction and statistical strength only; effect magnitudes are not clinically comparable",
    x = "Signed outcome statistic (beta/SE)", y = NULL, shape = NULL
  ) +
  theme_ijir(6.2) +
  theme(legend.position = "bottom", strip.text = element_text(size = 6.3),
        axis.text.y = element_text(size = 5.6))

s6_compression <- data.table(
  evidence_level = c(
    "Trait rows", "Lead variants", "LD-defined locus", "African-ancestry coverage"
  ),
  x = c(0.75, 1.85, 2.95, 1.85),
  y = c(0.67, 0.67, 0.67, 0.18),
  label = c(
    "8 FDR\ntrait rows",
    "3 correlated\nlead SNPs",
    "1 LD-defined\nLCT/MCM6 locus",
    "AFR: 2/8 rows estimable; 0 FDR"
  )
)
s6_source2 <- write_source(
  s6_compression,
...
s6b <- ggplot(s6_compression, aes(x, y, label = label)) +
  geom_segment(
    data = data.table(x = c(1.18, 2.28), xend = c(1.42, 2.52),
                      y = c(0.67, 0.67), yend = c(0.67, 0.67)),
    aes(x = x, xend = xend, y = y, yend = yend),
    inherit.aes = FALSE,
    arrow = grid::arrow(type = "closed", length = grid::unit(2.1, "mm")),
    linewidth = 0.55, colour = palette[["neutral_dark"]]
  ) +
  geom_label(
    data = s6_compression[evidence_level != "African-ancestry coverage"],
    size = 2.3, family = font_family, fontface = "bold",
    fill = palette[["neutral_pale"]], colour = palette[["neutral_dark"]],
    label.size = 0.35, label.padding = grid::unit(1.9, "mm")
  ) +
  geom_label(
    data = s6_compression[evidence_level == "African-ancestry coverage"],
    size = 2.25, family = font_family,
    fill = "white", colour = palette[["neutral_dark"]],
    label.size = 0.35, label.padding = grid::unit(1.8, "mm")
  ) +
  annotate(
    "text", x = 1.85, y = 0.39,
    label = "Separate non-overlapping ancestry-transfer sensitivity",
    size = 1.85, family = font_family, colour = palette[["neutral_mid"]]
  ) +
  coord_cartesian(xlim = c(0.25, 3.45), ylim = c(0.02, 0.92), clip = "off") +
  labs(
    tag = "b", title = "Evidence compression and outcome coverage summary",
    subtitle = paste0(
      "rs4988235 (5 rows), rs6754311 (2), rs7570971 (1); ",
      "pairwise EUR r² 0.859–0.972\n",
      "Boxes represent different evidential levels, not directly comparable counts"
    )
  ) +
  theme_void(base_family = font_family) +
  theme(
    plot.title = element_text(size = 7.3, face = "bold", hjust = 0),
    plot.subtitle = element_text(size = 5.7, hjust = 0, lineheight = 1.1),
    plot.tag = element_text(size = 9, face = "bold"),
    plot.margin = margin(5, 9, 3, 5)
  )

supp_s6 <- s6a / s6b + plot_layout(heights = c(1.48, 0.78)) +
  plot_annotation(caption = paste0(
    "The eight European and cross-ancestry trait-level associations surviving FDR correction are not eight independent biological findings.\n",
    "The three lead SNPs form one high-LD locus; African non-estimability reflects SNP coverage rather than evidence of absence."
  )) & theme(
    plot.caption = element_text(size = 5.8, hjust = 0, lineheight = 1.15,
                                margin = margin(t = 4, b = 6)),
    plot.margin = margin(4, 8, 10, 4)
  )
save_gg(
  supp_s6,
  "IJIR_Supplementary_Figure_S6_2025_ED_Locus_Ancestry_v0_3_5",
  183, 158, supplement_dir, c(s6_source1, s6_source2)
)

# Combined supplementary PDF and output manifest ------------------------------
combined_pdf <- file.path(package_root, "06_supplement",
                          "IJIR_Supplementary_Figures_v0_3_5.pdf")
grDevices::cairo_pdf(combined_pdf, width = 183 / 25.4, height = 158 / 25.4,
                     family = font_family, onefile = TRUE)
print(supp_s1); print(supp_s2); print(supp_s3); print(supp_s4); print(supp_s5); print(supp_s6)
grDevices::dev.off()
...
fwrite(manifest, file.path(qc_dir, "IJIR_Figure_Output_Manifest_v0_3_5.csv"))

cat(sprintf(
  paste0(
    "IJIR v0.3.5 figures complete: 3 main figures, 6 supplementary figures, ",
