...
await buildMainTable(
  "main_table_1.json", "IJIR_Table_1_Data_Sources_v0_3.xlsx",
  "Table 1. GWAS sources and their prespecified analytical roles",
  [32, 17, 20, 17, 45, 48, 44, 48, 44],
);
await buildMainTable(
  "main_table_2.json", "IJIR_Table_2_Forward_Nominal_Associations_v0_3.xlsx",
  "Table 2. Nominal forward gut microbiota-to-erectile-dysfunction associations",
  [19, 42, 18, 46, 18, 14, 18, 14, 23, 14, 14, 16, 46, 42, 30, 62, 48],
);

const supplementManifest = await readJson("supplement_manifest.json");
const wb = Workbook.create();
const previews = [];
...
await verifyAndExport(
  wb,
  path.join(supplementDir, "IJIR_Supplementary_Data_v0_3.xlsx"),
  previews,
);

console.log("Created IJIR v0.3 main-table and supplementary workbooks with previews.");
