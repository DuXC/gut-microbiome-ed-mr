...
function formatSheet(sheet, title, headers, values, widths = []) {
  ...
  headers.forEach((header, i) => {
    ...
    if (values.length && /^Minimum (?:P|q)$/i.test(header)) {
...
await buildMainTable(
  "main_table_1.json", "IJIR_Table_1_Data_Sources_v0_2.xlsx",
  "Table 1. GWAS sources and their prespecified analytical roles",
  [30, 14, 15, 42, 34, 36, 42],
);
await buildMainTable(
  "main_table_2.json", "IJIR_Table_2_Forward_Nominal_Associations_v0_2.xlsx",
...
const specs = [
  ...
  ["Mech Family Summary", "Mechanistic extension family-level results", supplement.mechanistic_summary, [42, 18, 14, 18, 16, 16]],
  ["Mech Family Denoms", "Frozen mechanistic family denominators", supplement.mechanistic_denominators],
  ["Mech X-Y", "Microbial-function to erectile-dysfunction total effects", supplement.mechanistic_x_to_y],
  ["Cyto M-Y", "Cytokine to erectile-dysfunction effects", supplement.cytokine_m_to_y],
  ["Cyto X-M", "Microbial-function to cytokine effects", supplement.cytokine_x_to_m],
  ["Cyto Indirect", "Cytokine-pathway indirect effects", supplement.cytokine_indirect],
  ["Endo M-Y", "Endothelial-protein to erectile-dysfunction effects", supplement.endothelial_m_to_y],
  ["Endo X-M", "Microbial-function to endothelial-protein effects", supplement.endothelial_x_to_m],
  ["Endo Indirect", "Endothelial-pathway indirect effects", supplement.endothelial_indirect],
  ["Endo Instruments", "Endothelial cis-pQTL instrument receipt", supplement.mechanistic_endothelial_instruments],
  ["Mech Source Metadata", "Mechanistic source metadata", supplement.mechanistic_source_metadata],
  ["Mech Overlap", "Mechanistic sample-overlap matrix", supplement.mechanistic_overlap],
  ["Mech Validation", "Mechanistic full-validation receipt", supplement.mechanistic_validation],
  ["Mech Decision", "Frozen mechanistic stopping decision", supplement.mechanistic_decision],
  ["Mech Raw Manifest", "Mechanistic raw-file manifest", supplement.mechanistic_raw_manifest],
...
await verifyAndExport(
  wb,
  path.join(supplementDir, "IJIR_Supplementary_Data_1_v0_2.xlsx"),
  previews,
);
